...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  irun_corr_factor,izero_snow_date,iclear_mn,iclear_dy,
...
c   running the multi-layer snow model.
      if (multilayer_snowpack.eq.1) then
        if (irun_corr_factor.eq.1 .or. ihrestart_flag.ne.-2 .or.
     &    snow_d_init_const.ne.0.0) then
          print *, 'The multi-layer snowpack model requires:'
          print *, '  irun_corr_factor = 0'
...
c   increments that are too large.
      if (deltax.gt.250.0 .or. deltay.gt.250.0) then
        if (run_snowtran.eq.1.0) then
          print *
          print *, '!!! deltax,y should not be greater than 250 m'
...
c   nearest-station indexing array.
      if (barnes_lg_domain.eq.1.0) then
        ...
        call get_nearest_stns(nx,ny,xmn,ymn,deltax,deltay,
     &    n_stns_used,k_stn)
      endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine get_nearest_stns(nx,ny,xmn,ymn,deltax,deltay,
     &  n_stns_used,k_stn)
