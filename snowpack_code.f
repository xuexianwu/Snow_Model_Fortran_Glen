...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  swe_depth_old,canopy_int_old,sprec_grnd_ml
...
c Distribute surface melt and rain precipitation through the snowpack.
     ...
     &  sfc_sublim_flag,sum_sfcsublim,swesublim)
...
c Account for the accumulation of snow precipitation on the snowpack.
     ...
     &  swe_lyr,ro_nsnow,sprec,T_old,Tsfc,tsls_threshold,dt,
...
c Calculate the temperature of each snow layer.
      CALL SNOWTEMP_ML(gamma,T_old,Tsfc,JJ,dt,ro_layer,Cp_snow,
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  xLsublim,potsublim,swesublim,sum_sfcsublim
...
c   as runoff.
      ...
      if (extra.gt.0.0) then
        do j=JJ,1,-1
          if (extra.gt.0.0) then
            swe_space = dy_snow(j) * (ro_snowmax - ro_layer(j)) /
     &        ro_water
            add = min(swe_space,extra)
            swe_lyr(j) = swe_lyr(j) + add
            extra = extra - add
          endif
...
c   was used to freeze and raise the snow temperature.
            delta_T = (extra * xLf) / (Cp_snow * dy_snow(j))
...
c   temperature in the presence of the liquid.
            ...
            extra_delta_T = max(0.0,T_old(j)-Tf)
...
c   a different size).
            extra = Cp_snow * dy_snow(j) * extra_delta_T / xLf
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  sum_prec,prec,sum_runoff,runoff,snow_d,snow_depth,swe_depth)
      ...
      real swe_lyr(nz_max)
      real T_old(nz_max)
      real ro_nsnow,sprec,ro_water,z_nsnow,tsls_threshold,
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  swe_depth,swe_depth_old,canopy_int_old,JJ,
     ...
     &  multilayer_snowpack,tsls_threshold)
      ...
      real tsls_threshold,ro_snow,Tf
      real tslsnowfall(nx_max,ny_max)
      real dy_snow(nx_max,ny_max,nz_max)
      real swe_lyr(nx_max,ny_max,nz_max)
      real ro_layer(nx_max,ny_max,nz_max)
