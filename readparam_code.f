...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  irun_corr_factor,lapse_rate_user_flag,
...
c         endif
     ...
     &        c_param(1:i_param_chars))
          ...
          if (c_param(1:i_param_chars).eq.'irun_corr_factor') then
            ipar_count(36) = 1
            call char2int(irun_corr_factor,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (irun_corr_factor.ne.0 .and. irun_corr_factor.ne.1) then
              print *,'irun_corr_factor not 0 or 1'
              stop
            endif
          endif
...
c MICROMET MODEL SETUP.
     ...
     &        then
              print *,'barnes_lg_domain not 0.0 or 1.0'
              stop
            endif
          endif
...
c   parameters have been defined.
      ipar_flag = 0
      do k=1,npars
        if (ipar_count(k).ne.1) then
          ipar_flag = ipar_flag + 1
          print *
          print *
          print *,'MISSING VARIABLE IN .PAR FILE: AT VAR LINE =',k
          print *
        endif
      enddo

      if (ipar_flag.ne.0) stop
