...
c   FAX: (970) 491-8241
c
c   liston@cira.colostate.edu

      implicit none

      include 'snowmodel.inc'

      include 'snowmodel.vars.inc'
...
c   iterations.
      ...
      do icorr_factor_loop=i_corr_start,irun_corr_factor+1

c Perform the correction (precipitation and melt) factor
c   calculations.
        if (irun_corr_factor.eq.1 .and. icorr_factor_loop.eq.2) then
...
c Distribute the meteorological station data.
     ...
     &        k_stn,xlat_grid,xlon_grid,UTC_flag)
...
c   during the simulation.
     ...
     &          (((T_old(i,j,k)-273.16,i=1,nx),j=1,ny),k=1,nz_max),
...
c Save the history restart information.
          if (ihrestart_flag.ge.-1) then
            if (mod(iter,ihrestart_inc).eq.0.0
